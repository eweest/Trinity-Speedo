...
local TEXTURES = {  -- NAME, URL
	...
	["ICON_RADIO"] = {"icon-radio.png", "https://github.com/eweest/Trinity-Speedo/raw/main/assets/textures/icon-radio.png"},
	["ICON_LIGHTS"] = {"icon-lights.png", "https://github.com/eweest/Trinity-Speedo/raw/main/assets/textures/icon-lights.png"},
}
-- FONTS
local FONTS = {
...
-- Radio Station List
local radioStationTable = { -- Textdraw text, Title
	{"OFF", "Выкл"},
	{"TRP2FM", "TRP2 FM"},
	{"radio~n~record", "Радио RECORD"},
	{"europa", "Europa Plus"},
	{"NRJ", "NRJ"},
	{"ROCK ~w~FM", "ROCK FM"},
	{"RECORD~n~BLACK", "RECORD Black"},
	{"Russian~n~Radio", "Русское Радио"},
	{"D~w~FM", "DFM"},
	{"Retro", "Ретро FM"},
	{"Dorozhnoe", "Дорожное радио"},
	{"Radio~n~Chanson", "Радио Шансон"},
	{"100 hitz", "100 Hitz"},
	{"Jazz", "Jazz Radio"},
	{"Soul", "Soul Radio"},
	{"Easy Hits~n~Florida", "Easy Hits Florida"},
	{"181 FM~n~Hip Hop", "181 FM Hip Hop"},
	{"Nashe~n~Radio", "Наше Радио"},
}
...
function main()
	...
	sampRegisterChatCommand("speedo", function(cmd) -- HELP
		...
		saveDB()
		reloadScript()
		elseif (cmd == "radio" or cmd == "rd") then -- UPDATE
			DB["RADIO"] = not DB["RADIO"]
			if DB["RADIO"] then
				DB["RADIO"] = true
				sendToChatMsg(string.format("Показ радио для ресурса {FFCC00}%s {32FF32}включено.", thisScript().name), "done")
				addOneOffSound(0.0, 0.0, 0.0, SOUNDS["DONE"])
			else
				DB["RADIO"] = false
				sendToChatMsg(string.format("Показ радио для ресурса {FFCC00}%s {FF3232}отключено.", thisScript().name), "done")
				addOneOffSound(0.0, 0.0, 0.0, SOUNDS["CANCEL"])
			end
		saveDB()
		...
		elseif cmd:find("^color") then -- CHANGE COLOR
			...
			if COLOR ~= nil and COLOR ~= "" and COLOR:len() == 6 then
				...
				sendToChatMsg(string.format("Вы ввели некорректно команду. Актуальный цвет [ {%s}#%s{FFFFFF} ], чтобы изменить введите: {FFCC00}%s color FFFFFF", DB[GET_SERVER]["COLOR"], DB[GET_SERVER]["COLOR"], CMD_SCRIPT), "error")
...
function createSpeedo()
	...
	local setVehSpeed = getCarSpeed(getVehHandle)*2.8 or 0 -- VEH SPEED
	...
	local setVehTurbo = getVehTurbo -- CHECK TURBO
    local getVehPointer = getCarPointer(getVehHandle) -- VEH LISGHTS (HDL)
    local vehFalgs = readMemory(getVehPointer + 1064, 1, false) -- VEH LIGHTS (ADRESS)
...
-- TABLE's
	...
	-- ICON LIGTHS
    if vehFalgs >= 64 then
		renderDrawTexture(loadTexture["ICON_LIGHTS"], posX - 20, posY + 116, 32, 32, 0, ALPHA["100"] .. COLORS["ICON"]["SETCOLOR"]) -- LIGHTS ICON
	else
		renderDrawTexture(loadTexture["ICON_LIGHTS"], posX - 20, posY + 116, 32, 32, 0, ALPHA["15"] .. COLORS["ICON"]["WHITE"]) -- LIGHTS ICON
	end
	-- ICON ENGINE
	...
	if DB["RADIO"] == true then
		if radioStationActual == "Выкл" then
			renderDrawTexture(loadTexture["ICON_RADIO"], posX - 20, posY + 244, 32, 32, 0, ALPHA["15"] .. COLORS["ICON"]["WHITE"]) -- VEH HP ICON
		else
			renderDrawTexture(loadTexture["ICON_RADIO"], posX - 20, posY + 244, 32, 32, 0, ALPHA["100"] .. COLORS["ICON"]["SETCOLOR"]) -- VEH HP ICON
		end
		rdDrawText(string.format("%s", radioStationActual), posX - 24, posY + 250, FONTS["FUEL"], "right", ALPHA["50"] .. COLORS["WHITE"]) -- VEH HP TITLE
	end
	----
end
...
{AFE7FF}/speedo radio (rd){ffffff} - Показать/Скрыть радио
...
function sampev.onShowTextDraw(id, data)
	...
	for _, radio in pairs(radioStationTable) do
		if data.text:find(radio[1]) then
			radioStationID = id
			radioStationActual = radio[2]
		end
	end
