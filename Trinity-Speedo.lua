...
-- BODY [START]
	while true do wait(0)
		...
		if TRINITYGTA == true then
			-- CREATE SPEEDO
			if isCharInAnyCar(PLAYER_PED) and sampTextdrawIsExists(2060) then
				...
				local getVehID = getCarModel(veh) -- VEH ID	
					for _, vehID in ipairs(VEH_IDs) do
						if getDriverOfCar(veh) == PLAYER_PED and getVehID == vehID then
							if not sampIsScoreboardOpen() and not sampTextdrawIsExists(invID) then
								if not sampTextdrawIsExists(chipID) then
									if DB[GET_SERVER]["VIEW"] then
